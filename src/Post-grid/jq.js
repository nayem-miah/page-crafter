jQuery( document ).ready( function ( $ ) {
	$( 'body' ).on(
		'change',
		'.nls-parent-term .nls-child-term input[type="checkbox"]',
		function () {
			var selected_author = '';
			var selected_publisher = '';
			$(
				'.book-author-filters .nls-parent-term .nls-child-term input:checkbox:checked'
			).each( function ( indexInArray, valueOfElement ) {
				selected_author += $( this ).val() + '-';
			} );
			$(
				'.book-publisher-filters .nls-parent-term .nls-child-term input:checkbox:checked'
			).each( function ( indexInArray, valueOfElement ) {
			...
			var nls_book_publisher_attr = $( '#nls-book-publisher' ).attr(
				'name'
			);
			if ( selected_author == '' ) {
			...
			if ( selected_publisher == '' ) {
				...
				$( '#nls-book-publisher' ).attr(
					'name',
					nls_book_publisher_attr
				);
